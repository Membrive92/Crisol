...
- **[PROJECT-GUIDE.md](PROJECT-GUIDE.md) — guía maestra para entender y continuar el proyecto sin contexto previo (empieza por aquí)**
- **[HANDOFF.md](HANDOFF.md) — dónde estamos HOY y qué sigue (se reescribe cada sesión)**
...
- [audits/](audits/) — auditorías (seguridad, arquitectura, UX, rendimiento). La más reciente:
  [2026-08-31 — revisión adversarial del plan PHASE-44.28](audits/2026-08-31-plan-44.28-adversarial-review.md)
  (41 hallazgos, 16 confirmados, 7 bloqueantes)
...
[`phases/phase-30-31-plan.md`](phases/phase-30-31-plan.md) (plan ejecutivo conjunto con
...
> (as-built) y [`improvements/phase-37-analysis-redesign-plan.md`](improvements/phase-37-analysis-redesign-plan.md) (plan).
...
| 44.28 | **El Veredicto es el informe del ejercicio.** Tercera iteración del feedback y la que cambia el MODELO: el Veredicto deja de juzgar con inventarios y pasa a NARRAR el año — ocho secciones fijas (negocio · caja · balance · rentabilidad · contabilidad · dividendo · resistencia · veredicto) que recorren toda métrica roja/ámbar o que cambió de banda, con su comparativa plurianual y su racha («rojo los cinco ejercicios», «por primera vez»), y las 4 preguntas plegadas como chip de su sección. Mockup de PepsiCo aprobado por el usuario como contrato de aceptación, compuesto desde el run real. Decisión 1B: «Evitar» por insolvencia exige que los DOS modelos coincidan (con un solo O, el segundo modelo no puede desmentir nunca). Causalidad sólo por identidad aritmética con gate numérico; veredicto final con simetría obligatoria (nunca una sola rama); retira `next_checks` —que además imprime «ya ha cruzado hacia el rojo» sobre métricas en ámbar (`report.py:523`)— y los dos `*_intro` | ⏳ plan aprobado, sin código | — |
...
> Plan de 44.28 en [`improvements/phase-44.28-annual-report-verdict-implementation-plan.md`](improvements/phase-44.28-annual-report-verdict-implementation-plan.md)
> — autocontenido para que lo implemente otra sesión: mockup-contrato, formas
> JSONB verificadas contra la BD, reglas de composición con sus gates, y el
> prerrequisito de que 44.27-E1 (amortización parcial) aterrice antes de leer
> MCD. Sustituye al diseño «La Lectura» (no implementar).
...
> **Numeración**: el plan de liquidación anticipada de deuda
> ([`improvements/phase-48-debt-early-settlement.md`](improvements/phase-48-debt-early-settlement.md))
> también se llama PHASE-48 y está sin abrir; al abrirlo hay que renumerarlo
> (propuesta: PHASE-49). La PHASE-48 entregada es ésta.
...
```
internal_docs/
├── PROJECT-GUIDE.md        # guía maestra (vivo) — empieza por aquí
├── HANDOFF.md              # dónde estamos HOY (vivo, se reescribe cada sesión)
├── README.md               # este archivo: índice + tabla de estado de fases
├── architecture.md         # arquitectura del sistema (vivo)
├── development-spec.md     # metodología, plantilla de phase doc, definition of done
├── lessons.md              # errores reales y reglas aprendidas (leer entero)
├── backlog.md              # deuda técnica durable y follow-ups (vivo)
├── investment-module-guide.md            # guía + playbook manual del módulo Inversión
├── investment-threshold-divergences.md   # cuaderno del usuario vs motor
├── api/endpoints.md        # catálogo de endpoints
├── data-model/schema.md    # migraciones y tablas
├── decisions/              # ADRs 0001-0011
├── phases/                 # una foto fechada por fase entregada
├── improvements/           # planes y diseños (fotos fechadas)
├── audits/                 # auditorías y revisiones adversariales
├── design-explorations/    # wireframes y exports de Stitch
└── ai-context/             # contexto bajo demanda (transcripción del cuaderno del usuario)
```
...
- **Al llegar sin contexto**: `PROJECT-GUIDE.md` y después `HANDOFF.md`.
...
- **Al terminar una fase**: crea `phases/phase-X.Y-*.md`, actualiza las tablas
  de arriba, añade lo que corresponda a `api/` y `data-model/`, y reescribe
  `HANDOFF.md` (lo durable, a `backlog.md`). `python scripts/check_docs.py`
  tiene que quedar en verde.
