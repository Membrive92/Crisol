...
| GET    | `/dashboard/by-category`                              | `currency` o `target_currency`, `date_from?`, `date_to?`, `kind?` (`income\|expense`)                                | `[{ category_id, category_name, category_kind, total, count, deferred_total }]` — `deferred_total` (PHASE-47.E4) es la parte de `total` APLAZADA por un recibo financiado. Repartido por categorías suma exactamente `summary.deferred_expenses` del mismo rango                                                                                                                                                                   |
...
| GET    | `/dashboard/top-expenses`                             | `currency` o `target_currency`, `date_from?`, `date_to?`, `limit` (1..50, def 10)                                    | `[{ transaction_id, description, amount, flow, occurred_at, category_id, category_name, original_amount, original_currency }]` (PHASE-8.4: `original_*` siempre presentes; `amount` es el convertido en cross-currency, original en legacy. PHASE-47.H: `flow` es la DIRECCIÓN del movimiento — `amount` viaja SIN signo porque es el que ordena el ranking, así que sin `flow` la lista no puede pintar que una devolución RESTA) |
...
- **El día NUNCA viaja en la query**: sale de `users.cycle_start_day` (el perfil
  que `CurrentUser` ya trae en cada request). El flag sólo dice _qué_ pregunta
...
- `top-expenses` devuelve el **cubo de gasto** (`_is_expense()`): con
  `flow` explícito, las salidas y las **devoluciones** (una entrada en
  categoría de gasto, PHASE-47.H); sin `flow`, cae a `category.kind ==
expense`. Las devoluciones entran a propósito —son gasto de su
  categoría, con signo contrario— y por eso el item lleva `flow`.
...
- `exceptional_by_category[]` lleva `deferred_total` (PHASE-47.E4) por el
  mismo motivo que `by-category`: sin él, el filtro Fijo/Variable del desglose
  no puede repartir lo aplazado y su aviso vuelve a citar el periodo entero.
- `top_exceptional[]` sale del **cubo de gasto** (`_is_expense()`), que
  INCLUYE las devoluciones, y ordena por el importe SIN signo. Cada item
  lleva `flow` (PHASE-47.H) porque sin él un reembolso encabeza la lista
  como si fuera el mayor gasto del periodo, mientras `exceptional_by_category`
  —de la misma respuesta— lo RESTA de su categoría.
...
| Método | Ruta                                                  | Descripción                                                                                                                                                                                                                                                        |
| ------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GET    | `/investment/fundamentals/items`                      | **(44.9)** Las 49 partidas canónicas con etiqueta ES, `statement` (balance/income/cashflow) y `group`. **(44.23)** Además `help`: qué es la partida, si la publica la empresa o la deduce la app, y su convención de signo (`fundamentals/glossary.py`). Estático. |
