...
async def _setup_user(
    client: AsyncClient, email: str = "ba@example.com"
) -> tuple[str, str]:
    r = await client.post(
        "/auth/register",
...
async def test_no_budget_returns_no_alert(client: AsyncClient) -> None:
    token, cat_id = await _setup_user(client, "ba1@example.com")
...
async def test_under_warning_threshold_no_alert(client: AsyncClient) -> None:
    ...
    token, cat_id = await _setup_user(client, "ba2@example.com")
    await client.post(
        "/budgets",
        json={
            "category_id": cat_id,
...
async def test_warning_threshold_emits_alert(client: AsyncClient) -> None:
    ...
    token, cat_id = await _setup_user(client, "ba3@example.com")
    await client.post(
        "/budgets",
        json={
            "category_id": cat_id,
...
async def test_over_threshold_emits_alert(client: AsyncClient) -> None:
    ...
    token, cat_id = await _setup_user(client, "ba4@example.com")
    await client.post(
        "/budgets",
        json={
            "category_id": cat_id,
...
async def test_global_budget_triggers_alert(client: AsyncClient) -> None:
    ...
    token, cat_id = await _setup_user(client, "ba5@example.com")
    await client.post(
        "/budgets",
        json={
            "amount": "100.00",
...
async def test_falls_through_to_global_when_category_budget_ok(
    ...
    token, cat_id = await _setup_user(client, "ba6@example.com")
