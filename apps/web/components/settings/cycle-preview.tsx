...
/**
 * C2 — La previsualización del corte. **Esto ES la feature.**
 *
  ...
  return (
    <div>
      ...
      >
        ...
        Si tu nómina no aparece donde esperas, el día no es el que creías: cámbialo
        y vuelve a mirar.
      </p>
      {/*
...
function PreviewColumn({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: fontSize.xs, color: colors.textMuted }}>{subheading}</div>
      <div style={{ fontSize: fontSize.xs, color: colors.textMuted }}>{rangeLabel}</div>

      {isLoading ? (
        <p style={{ margin: `${spacing.sm}px 0 0 0`, fontSize: fontSize.sm, color: colors.textMuted }}>
          Cargando movimientos…
        </p>
      ) : error ? (
        <p
          role="alert"
          style={{ margin: `${spacing.sm}px 0 0 0`, fontSize: fontSize.sm, color: colors.danger }}
        >
          {error}
        </p>
      ) : (
        <>
          ...
          >
            {movementCountLabel(total)}
          </div>
          {rows.length === 0 ? (
            <p style={{ margin: `${spacing.xs}px 0 0 0`, fontSize: fontSize.sm, color: colors.textMuted }}>
              Ningún movimiento en este tramo.
            </p>
          ) : (
            <ul style={{ listStyle: 'none', margin: `${spacing.xs}px 0 0 0`, padding: 0 }}>
              {rows.map((tx) => (
...
function movementCountLabel(total: number | null): string {
  ...
  if (total === 1) return '1 movimiento cae en este ciclo';
  return `${total} movimientos caen en este ciclo`;
}
  ...
  return (
    ...
    >
      <span style={{ minWidth: 0, flex: 1 }}>
        <span style={{ fontSize: fontSize.xs, color: colors.textMuted, marginRight: spacing.xs }}>
